...
\begin{proposition}[Text ecology is bounded by linguistic equivalence]\label{prop:ling-bound}
...
\begin{corollary}[ICL strengthens the case for training-time veridicality]
If $(w_1,w_2) \notin S(\theta)$, then no prompting strategy can make the frozen model distinguish that pair. Therefore, whenever a deployment ecology assigns positive separation weight to such a pair, a strictly positive excess token loss is unavoidable for that frozen $\theta$ (by \thmref{thm:ce-decomposition}(c)).
\end{corollary}
...
\begin{remark}[Separation set versus ecology-relative encoding]
The full readout repertoire $S(\theta)$ from \defref{def:repertoire} records which pairs are distinguishable by \emph{some} context in $V^*$. The ecology-relative partition $\sim_{\theta,D}$ is coarser: if a pair is separated only on a $D_C$-null set of contexts, then $(w_1,w_2) \in S(\theta)$ but $w_1 \sim_{\theta,D} w_2$. Thus $S(\theta)$ can be strictly larger than what the training or deployment ecology actually exposes. This is the model-side analogue of \corref{cor:text-separation}: zero-measure distinguishing contexts do not affect the ecology-induced partition.
\end{remark}

Chain-of-thought prompting and scratchpads can still improve performance within a frozen model by generating intermediate tokens that create longer, more informative contexts \citep{wei_etal_2022_cot,nye_etal_2021_scratchpads}. In our terms, such procedures can make weak but already available distinctions easier to exploit at inference time, without enlarging the underlying separation set $S(\theta)$. This paper studies the representational question: which distinctions the frozen model can in principle preserve under an ecology. The separate computational question of how much inference-time work is needed to exploit an existing separation is not addressed here.
...
& \cdot \max\!\left\{
      \log(1/\rho(p^v))+\log(2/\alpha),\,
      \max_{p\ \text{non-veridical}}
      \Bigl(\log(1/\rho(p))+\log(2/\alpha)\Bigr)
...
\]
where $Z_{\beta_0}$ is the normalizing constant. Under that choice, low-complexity induced partitions receive larger mass and therefore tighter concentration radii. If the model class contains a minimum-complexity veridical partition, its contribution is governed by $\beta_0 I^*(\mu_D)+\log Z_{\beta_0}$ from \thmref{thm:min-complexity}. The exact sample bound still depends on the full maximum over non-veridical partitions; in general it cannot be reduced to the gap-achieving partition alone without extra structure relating $\Delta_D(p)$ to $H(p(W))$.

This theorem is still a uniform-convergence result over induced encodings, not a theorem about SGD on transformer parameter space. However, unlike the earlier ecological-risk formulation, the objective here is the actual token-level log-loss. The remaining idealisation is that each induced encoding $p_{\theta,D}$ is paired with its Bayes-optimal decoder $q_{\theta}^*$. The theorem therefore decomposes success into representation choice plus decoder optimality, and then into optimisation error $\varepsilon_{\mathrm{opt}}$ and statistical error $\eta$. What we do \emph{not} yet have is a separate realistic bound on the gap between the Bayes-optimal decoder $q_{\theta}^*$ and the decoder actually implemented by a trained transformer during a single training run; in the present theorem that gap is absorbed into the optimisation idealisation. The finite induced class assumption is automatically satisfied since $W$ is finite, and $M_\Theta$ cannot exceed the Bell number $B(|W|)$. But $B(|W|)$ grows super-exponentially; already $B(20) \approx 5.2 \times 10^{13}$. So the bound is mainly conceptual unless the effective induced class is far smaller than the worst-case partition count and the token-loss gap $\gamma_D^{\CE}$ is not too small. The same entropy $H(p(W))$ now appears in three distinct roles: as the minimum-complexity target in \thmref{thm:min-complexity}, as the explicit simplicity term in $J_{D,\beta}$ below, and, after choosing an entropic prior, as the statistical price paid to certify a partition from finite data.
...
\begin{proposition}[Importing ecological-veridicality dynamics to model populations]\label{prop:llm-wf}
Consider an idealized model-lineage population over a time window on which:
\begin{enumerate}[label=(\alph*)]
\item each active lineage $i$ carries a frozen deployment encoding $p_i$;
\item expected fitness is frequency-independent and depends on the encoding only through deployment performance, e.g.\ $\mathcal{F}(p_i) = C - \Delta_D(p_i)$ or any strictly decreasing transform of $\Delta_D(p_i)$, where $\Delta_D(p_i):=L_D^*(p_i)-H(Y \mid C,W)$;
...
\begin{theorem}[Minimum-complexity ecological veridicality]\label{thm:min-complexity}
...
\begin{corollary}[Topological convergence of minimum-complexity optima]\label{cor:topological-convergence}
...
Under the complexity-regularized objective $J_{D,\beta}$, \thmref{thm:split-threshold} shows that distinctions whose predictive Jensen--Shannon gain is smaller than $\beta\,h(\lambda)$ are locally preferred merge candidates. The pairs with the smallest gain-to-cost ratio are the first to become unstable as $\beta$ increases (\corref{cor:veridical-local-min}).
...
\begin{theorem}[Generalist advantage under shared separation]\label{thm:gen-vs-spec}
For each task $t \in \{1,\ldots,T\}$, let $D^{(t)}$ be the corresponding data distribution, let $\mu_t$ be its induced task ecology, and let $D_C^{(t)}$ be its context marginal. Define the excess Bayes-optimal token loss
\[
...
Let $\mathcal{R}$ be a finite recipe space and let $\alpha\colon\mathcal{R}\to[0,1]$ be a heritable scalar recipe trait, interpreted as the strength of ecology injection. Consider one selection stage in a Wright--Fisher population over recipes $r_1,\ldots,r_K$ with recipe frequencies~$x(r)$ and expected fitness~$f(r)$. Define the population mean trait $\bar\alpha := \sum_r x(r)\,\alpha(r)$ and let $\bar\alpha_{\mathrm{eval}}$ denote the mean trait in the selected-parent population. Then:
\begin{enumerate}[label=(\alph*)]
